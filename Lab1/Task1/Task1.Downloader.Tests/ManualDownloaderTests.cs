...
public sealed class ManualDownloaderTests : IDisposable
{
    ...
    {
        var result = await RunAsync(new StubHandler(HttpStatusCode.OK, Manual), "flashrom");
    ...
    public async Task OverwritesExistingFiles()
    {
        ...
        var result = await RunAsync(new StubHandler(HttpStatusCode.OK, Manual), "Done");
    ...
    private sealed class StubHandler(HttpStatusCode status, string body) : HttpMessageHandler
    {
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken) =>
            Task.FromResult(new HttpResponseMessage(status) { Content = new ByteArrayContent(Encoding.ASCII.GetBytes(body)) });
    }
}
