import { PAGE_SIZE, STATUS_SCALE_KEYS } from '@labwatch/shared';
...
/** Page 1 without an anchor; pages 2…N send back the anchor that page 1 returned. */
const paging = {
  page: z.number().int().min(1).max(100_000).default(1),
  pageSize: z.number().int().min(5).max(100).default(PAGE_SIZE),
  anchor: z
    .object({ at: z.string().datetime({ offset: true }), key: z.string().min(1).max(100) })
    .nullable()
    .default(null),
};

/** A paged list with an optional branch filter: null = all branches ("Overview"). */
const pagedByBranch = z.object({ branch: z.string().min(1).max(255).nullable().default(null), ...paging }).prefault({});

const paged = z.object(paging).prefault({});
...
export const appRouter = t.router({
  ...
  ciRuns: t.procedure.input(pagedByBranch).query(({ ctx, input }) => ctx.api.ciRuns(input)),
  ...
  /** `area`: "Lab 1", "Infra", "CI", "Repo" — only commits touching it. */
  commits: t.procedure
    .input(
      z
        .object({
          branch: z.string().min(1).max(255).nullable().default(null),
          area: z.string().min(1).max(32).nullable().default(null),
          ...paging,
        })
        .prefault({}),
    )
    .query(({ ctx, input }) => ctx.api.commits(input)),

  pulls: t.procedure.input(pagedByBranch).query(({ ctx, input }) => ctx.api.pulls(input)),
  ...
  events: t.procedure.input(paged).query(({ ctx, input }) => ctx.api.events(input)),
  ...
  incidents: t.procedure.input(paged).query(({ ctx, input }) => ctx.api.incidents(input)),
